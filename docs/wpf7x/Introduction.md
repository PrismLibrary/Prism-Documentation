...
- **Dependency injection container**. The Dependency Injection (DI) pattern is used throughout Prism to allow the dependencies between components to be managed. Dependency injection allows component dependencies to be fulfilled at run time, and it supports extensibility and testability. Prism currently works with Unity, Dryloc, Ninject and StructureMap. You could also extend Prism to work with another DI container if you wish.
- **Dependency injection container**. The Dependency Injection (DI) pattern is used throughout Prism to allow the dependencies to be fulfilled at run time, and it supports extensibility and testability. Prism currently works with Unity, Dryloc, Ninject and StructureMap. You could also extend Prism to work with another DI container if you wish. 
...
- **Bootstrapper**. This is the App object in the application. It is derived from PrismApplication. There are multiple versions of the PrismApplication class that can be derived from: these are based on the dependency injection container that is selected. This class will initialize that dependency injection container and any services that Prism uses, such as logging. At a minimum the application must implement methods to create the main window and register any services that the application needs.
...
The bootstrapper is the glue that connects the application with the Prism Library services and the selected dependency injection container. In the application, the App object will derive from the dependency injection container enabled PrismApplication base class (Prism.Unity.PrismApplication, Prism.Dryloc.PrismApplication, etc). In the derived class, some abstract methods will need to be implemented, CreateShell and RegisterType. In addition, if needed, the application will need a method of populating the module catalog. At a minimum, each application will provide a module catalog and a shell.

By default the bootstrapper logs events using the .NET Framework **Console.Out**. Most applications will want to supply a specific logging service. This can be specified in the bootstrapper.
