...
The Raygun Implementation of the Logger makes reference to the [Prism.Plugin.Essentials](../../essentials/index.md) library. This allows us to dynamically check if various Essentials interfaces have been registered. If they have we automatically tailor the Logger to include offline support with request caching. Additionally we will automatically update the configuration to make use of the [Application Context](../../essentials/applicationmodel/appcontext.md) from Essentials. In order to enable sending logs in the background with the offline store, be sure to Register Prism.Plugin.Essentials first.
